...
public class CampaignrecordsController implements Job{
	...
	public JsonResult<Map<String, Object>> partTake(String volunteerId,String campaignId,String token,String keyParams){
		...
		if(listVolun == null || listVolun.size() <= 0){
			return new JsonResult<Map<String, Object>>(1,"志愿者信息不存在",null);
		}
		if(!listVolun.get(0).getStatus().equals(YCConstants.AUDIT_TURE)){
			return new  JsonResult<Map<String, Object>>(DefaultResponse.NOT_PASS_VOLUN,null);
		}
				...
				try{
					...
					switch(partStatus){
						...
						case "5":
							...
							break;
						case "6":
							jsonResult = new JsonResult<Map<String,Object>>(DefaultResponse.NO_PART_EXIST,returnMap);
							break;
					}
				} catch (Exception e) {
					e.printStackTrace();
